...
export default function InvoiceDetailPage() {
  ...
  const { data: invoice } = useFrappeGetDoc("Sales Invoice", name ?? "");
  ...
  const linkedSalesOrder = items.find((item: { sales_order?: string }) => item.sales_order)?.sales_order;
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {items.map((item: { item_name: string; qty: number; amount: number }, i: number) => (
