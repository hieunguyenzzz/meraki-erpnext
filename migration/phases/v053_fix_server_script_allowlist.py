"""Fix Server Script allowlist: add missing commission and dependents fields.

v052 failed to update the Server Script because it used underscores
('meraki_set_employee_fields') instead of the actual hyphenated name
('meraki-set-employee-fields'). This phase corrects that by rewriting the
ALLOWED_FIELDS set in the script to match employee.py's ALLOWED_FIELDS.
"""


# Canonical allowlist — must match webhook_v2/routers/employee.py ALLOWED_FIELDS.
ALLOWED_FIELDS_SET = """{
    "employee_name",
...
SCRIPT_NAME = "meraki-set-employee-fields"  # hyphens, not underscores


def run(client):
    script = client.get("Server Script", SCRIPT_NAME)
    if not script:
        print(f"  ERROR: Server Script '{SCRIPT_NAME}' not found — cannot update allowlist")
        return

    current = script.get("script", "")

    # Check if all new fields are already present
    new_fields = [
    ...
    if all(f in current for f in new_fields):
        print("  Server Script already has all required fields, skipping")
        return

    result = client.update("Server Script", SCRIPT_NAME, {"script": FULL_SCRIPT})
    if result:
        print("  Updated Server Script allowlist with commission + dependents fields")
    else:
