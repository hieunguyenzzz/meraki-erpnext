...
export function RecruitingCard({ item, onOpenSheet }: RecruitingCardProps) {
  if (onOpenSheet) {
    return (
      <div
        className="rounded-lg border bg-card p-3 space-y-1.5 hover:shadow-sm cursor-pointer"
        onClick={(e) => { e.stopPropagation(); onOpenSheet(item.id); }}
